...
public class boxChecker extends sudoku implements Runnable{
    ...
    private void checkBox() {
        for (int i = this.startRowbox; i < this.startRowbox + 3; i++) {
            template.Template.instrum(27, "For", new template.Pair("i",i), new template.Pair("this.startRowbox",this.startRowbox));
			for (int j = this.startColumnbox; j < this.startColumnbox + 3; j++) {
                template.Template.instrum(29, "For", new template.Pair("j",j), new template.Pair("this.startColumnbox",this.startColumnbox));
