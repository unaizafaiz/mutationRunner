...
public class gamePlay {
                ...
                if (playingMode != CHALLENGE_MODE) {
                    if (sudokuOperation(CHECK_SUDOKU)) {
                        ...
                        if (isSudokuValid(userSudoku)) {
                            ...
                            for (int rowCounter = 0; rowCounter < 9; rowCounter++) {
                                template.Template.instrum(236, "For", new template.Pair("rowCounter",rowCounter));
								for (int columnCounter = 0; columnCounter < 9; columnCounter++) {
                                    template.Template.instrum(238, "For", new template.Pair("columnCounter",columnCounter));
                            ...
                            if (playingMode == NEW_GAME_MODE || playingMode == LOAD_GAME_MODE) {
                                ...
                                if (bestScores.size() > 0) {
                                    for (int counter = 0; counter < bestScores.size(); counter++) {
                                        template.Template.instrum(265, "For", new template.Pair("counter",counter));
                    ...
                    if (Sudoku.solveSudoku()) {
                        ...
                        if (isSudokuValid(computerSolution)) {
                            for (int rowCounter = 0; rowCounter < 9; rowCounter++) {
                                template.Template.instrum(299, "For", new template.Pair("rowCounter",rowCounter));
								for (int columnCounter = 0; columnCounter < 9; columnCounter++) {
                                    template.Template.instrum(301, "For", new template.Pair("columnCounter",columnCounter));
            ...
            if (hintDetails == null) {
                ...
                if (redoHistoryMoveNumber != history.size()) {
                    for (int counter = history.size() - 1; counter >= redoHistoryMoveNumber; counter--) {
                        template.Template.instrum(475, "For", new template.Pair("counter",counter), new template.Pair("redoHistoryMoveNumber",redoHistoryMoveNumber));
        ...
        solveButton.setOnAction(e -> {
            ...
            try {
                if (isSudokuValid(computerSolution)) {
                    ...
                    for (int rowCounter = 0; rowCounter < 9; rowCounter++) {
                        template.Template.instrum(522, "For", new template.Pair("rowCounter",rowCounter));
						for (int columnCounter = 0; columnCounter < 9; columnCounter++) {
                            template.Template.instrum(524, "For", new template.Pair("columnCounter",columnCounter));
							sudokuCells[rowCounter][columnCounter].setText(computerSolution[rowCounter][columnCounter] + "");
                            sudokuCells[rowCounter][columnCounter].setEditable(false);
                            sudokuCells[rowCounter][columnCounter].getStyleClass().add("cell-success");
                        }
    ...
    private void initSudokuBlock() {
        ...
        for (rowCounter = 0; rowCounter < 9; rowCounter++) {
            template.Template.instrum(635, "For", new template.Pair("rowCounter",rowCounter));
			for (columnCounter = 0; columnCounter < 9; columnCounter++) {
                template.Template.instrum(637, "For", new template.Pair("columnCounter",columnCounter));
                    ...
                    if (currentField.getLength() > 1) {
                     ...
                     if (listenToChange && playingMode == NEW_GAME_MODE || listenToChange && playingMode == LOAD_GAME_MODE) {
                            ...
                            if (redoHistoryMoveNumber != history.size()) {
                                ...
                                for (int counter = history.size() - 1; counter >= redoHistoryMoveNumber; counter--) {
                                    template.Template.instrum(703, "For", new template.Pair("counter",counter), new template.Pair("redoHistoryMoveNumber",redoHistoryMoveNumber));
									history.remove(counter);
                                }
                            }
    ...
    private void saveCurrentGame() {
        ...
        for (int rowCounter = 0; rowCounter < 9; rowCounter++) {
            template.Template.instrum(848, "For", new template.Pair("rowCounter",rowCounter));
			for (int columnCounter = 0; columnCounter < 9; columnCounter++) {
                template.Template.instrum(850, "For", new template.Pair("columnCounter",columnCounter));
    ...
    private Boolean isSudokuValid(Integer[][] sudoku) throws InterruptedException {
        ...
        for (int rowCounter = 0; rowCounter < 9; rowCounter++) {
            template.Template.instrum(882, "For", new template.Pair("rowCounter",rowCounter));
			for (int columnCounter = 0; columnCounter < 9; columnCounter++) {
                template.Template.instrum(884, "For", new template.Pair("columnCounter",columnCounter));
				sudokuCells[rowCounter][columnCounter].getStyleClass().remove("cell-danger");
    ...
    static Boolean sudokuOperation(int opType) {
        for (int rowCounter = 0; rowCounter < 9; rowCounter++) {
            template.Template.instrum(907, "For", new template.Pair("rowCounter",rowCounter));
			for (int columnCounter = 0; columnCounter < 9; columnCounter++) {
                template.Template.instrum(909, "For", new template.Pair("columnCounter",columnCounter));
				switch (opType) {
                    //Read Sudoku
                    case 1:
    ...
    private void highlightCell(String numberMatch) {
        for (int rowCounter = 0; rowCounter < 9; rowCounter++) {
            template.Template.instrum(976, "For", new template.Pair("rowCounter",rowCounter));
			for (int columnCounter = 0; columnCounter < 9; columnCounter++) {
                template.Template.instrum(978, "For", new template.Pair("columnCounter",columnCounter));
